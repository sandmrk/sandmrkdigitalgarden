...
<style>
  ...
  #tag-title { font-size: 2.8rem !important; font-weight: 900 !important; color: var(--cg-text) !important; margin: 15px 0 !important; }
...
<div id="cgfan-tag-page">
    ...
    <div id="tag-results">
        <div style="text-align:center; padding:80px; color:rgb(153,153,153);">正在清洗数据并加载预览...</div>
    </div>
</div>
    ...
    try {
        ...
        filtered.forEach(item => {
            ...
            if (imgUrl) {
                ...
                // 2. 针对 Twitter 链接的“暴力美学”清洗
                if (cleanUrl.includes('twimg.com')) {
                    // 砍掉 ? 及其后面的一切，砍掉 : 及其后面的一切
                    cleanUrl = cleanUrl.split('?')[0].split(':')[0] + ':' + cleanUrl.split('?')[0].split(':')[1];
                    // 如果被截断后没了后缀，手动补一个 .jpg，Weserv 会自动处理剩下的
                    if (!cleanUrl.match(/\.(jpg|png|webp|jpeg)$/i)) {
                        cleanUrl = cleanUrl.replace(/\/$/, '') + ".jpg";
                    }
                    finalImg = `https://images.weserv.nl/?url=${encodeURIComponent(cleanUrl)}&w=600&il`;
                } else {
                    finalImg = `https://images.weserv.nl/?url=${encodeURIComponent(cleanUrl)}&w=600`;
                }
            } else {
            ...
            <div class="tag-card">
                <a href="${item.url}" class="tag-card-img">
                    <img src="${finalImg}" loading="lazy" onerror="this.onerror=null;this.src='https://via.placeholder.com/600x400?text=Wait+Image';">
                </a>
